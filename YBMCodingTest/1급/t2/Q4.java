...
public class Q4 {
    ...
    public int solution(int[] arr, int K) {
        ...
        dfs(arr, visited, result, K, 3, 0, 0);
        answer = result[0];
        return answer;
    }

    private void dfs(int[] arr, boolean[] visited, int[] result, int K, int cnt, int sum, int idx) {
        if(cnt == 0){
            if(sum % K == 0){
                result[0]++;
            }
        ...
        for (int i = idx; i < arr.length; i++) {
            ...
            visited[i] = true;
            dfs(arr,visited,result,K,cnt-1, sum+arr[i], i+1);
            visited[i] = false;
        }
    }
